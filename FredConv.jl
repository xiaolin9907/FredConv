...
using LinearAlgebra, BandedMatrices, SparseArrays, FillArrays
...
##########################################################################################################
function fred_conv(fe, ge, r)
    # Return the result of Fredholm convolution without the spectral approximation matrix T.
    ...
    # 1. when r >= 1
    if r >= 1
        # Store elements located near the region boundary. 
        d = zeros(Ty,n,2)
        d2 = zeros(Int,n,1)
        ...
        axpy!(ge[1], vOld, gamma)
        axpy!(ge[2], view(v,2:n-1), view(gamma, 2:n-1))
        
        view(d,1,2) .= view(vOld,1)
        ind = min(floor(Int, 3*r)-1,n)
        view(d,2:ind,1) .= view(vOld,2:ind)
        view(d,2:ind,2) .= view(v,2:ind)
        view(d2,1) .= 1
        view(d2,2:ind) .= 2
        
        # Constructing the region S1 by (2.2a) in Theorem 2.3
        S = BandedMatrix(-1 => 1 ./ (2*(1:n-1).-1), 0 => Fill(0, n), 1 => -1 ./ (2*(1:n-1) .+ 1))      
        for j = 3:ceil(Int, (n+1)/(r+1))
            i = floor(Int, j*r):n+1-j
            if length(i) >0
                # Recurrence
                view(vNew,i) .= view(S,i,floor(Int, j*r)-1:n+2-j)*view(v,floor(Int, j*r)-1:n+2-j)
                broadcast!(*, view(vNew,i), r*(2*j - 3), view(vNew,i))
                broadcast!(+, view(vNew,i), view(vOld,i), view(vNew,i))
                # append to gamma
                axpy!(ge[j], view(vNew,i), view(gamma,i))
                
                # elements located near the region boundary.
                ind = min(n,floor(Int, j*r)):min(floor(Int, (j+1)*r)-1,n)
                view(d,ind, 1) .= view(v, ind)
                view(d,ind, 2) .= view(vNew, ind)
                view(d2,ind) .= j

                copyto!(view(vOld,i),view(v,i))
                copyto!(view(v,i),view(vNew,i))
            end
        end
        ind = findlast(d2 .!= 0)
        view(d2, ind[1]+1:n) .= view(d2, ind[1]) 
        view(d, ind[1]:n,2) .= view(v, ind[1]:n)
        ...
        # Constructing the region S23 by (2.2d) in Theorem 2.3
        indrow = min(n-2, n+1-floor(Int, (n+1)/(r+1)))
        vOld = zeros(Ty,1,n)
        vNew = zeros(Ty,1,n)
        v = zeros(Ty,1,n)
        S = BandedMatrix(1 => (1/r) ./ (2*(1:n-1).-1), 0 => Fill(0, n), -1 => (-1/r) ./ (2*(2:n) .- 1))
        for i = indrow:-1:2
            j = d2[i]+1:n+1-i  # i < j * r   
            if length(j) >0
                # Recurrence
                view(vOld,d2[i+2]-1:d2[i+2]) .= view(d, i+2,1:2)
                view(v,d2[i+1]-1:d2[i+1]) .= view(d, i+1, 1:2)
                view(vNew,j) .= view(S,j,d2[i]:n+2-i )*view(v,d2[i]:n+2-i)
                broadcast!(*, view(vNew,j), 2*i-1, view(vNew,j))
                axpy!((2*i - 1)/(2*i + 3), view(vOld,j), view(vNew,j))
                # append to gamma
                view(gamma,i) .+= dot(view(vNew, j), view(ge,j))

                copyto!(view(vOld,j),view(v,j))
                copyto!(view(v,j),view(vNew,j))
            end
        end

        # Constructing the first row
        j = 2:n-1 
        view(vOld,d2[3]-1:d2[3]) .= view(d, 3,1:2)
        view(v,d2[2]-1:d2[2]) .= view(d, 2, 1:2)
        view(vNew,j) .= view(S,j,1:n)*view(v,1:n)
        axpy!(1/5, vOld, vNew)
        view(vNew,n) .= -1 ./ (r*(2*n - 1))*(view(v, n - 1))
        view(gamma,1) .+= dot(view(vNew,2:n), view(ge,2:n))
    end
    ...
    if r < 1
        ...
        view(gamma,1) .+= dot(vOld, ge)
        view(gamma,2) .+= dot(view(v,2:n-1), view(ge,2:n-1))
        view(d,1,1) .= view(vOld, 1)
        view(d,2:floor(Int, 3/r)-1,1) .= view(v, 2:floor(Int, 3/r)-1)    
        view(d,2:floor(Int, 3/r)-1,2) .= view(vOld, 2:floor(Int, 3/r)-1)
        view(d2,1) .= 1
        view(d2,2:floor(Int, 3/r)-1) .= 2
        ...
        for i = 3:ceil(Int, (n+1)/(1+1/r))
            ...
            if length(j) >0
                # Recurrence
                view(vNew,j) .= view(S,j,floor(Int, i/r)-1:n+2-i) * view(v,floor(Int, i/r)-1:n+2-i)
                broadcast!(*, view(vNew,j), (2*i - 1), view(vNew,j))
                axpy!((2*i - 1)/(2*i - 5), view(vOld,j), view(vNew,j))
                # append to gamma
                view(gamma, i) .+= dot(view(vNew,j), view(ge,j))

                # elements located near the region boundary.
                ind = floor(Int, i/r):floor(Int, (i+1)/r)-1
                view(d,ind,1) .= view(vNew,ind)
                view(d,ind,2) .= view(v,ind)
                view(d2,ind) .= i
                copyto!(view(vOld,j),view(v,j))
                copyto!(view(v,j),view(vNew,j))
            end
        ...
        ind = findlast(d2 .!= 0)
        view(d2, ind[1]+1:n) .= view(d2, ind[1])
        view(d, ind[1]+1:n,1) .= view(v, ind[1]+1:n) 
        for k = 1:n
            if d2[k] + k > n+1
                view(d,k,1) .= 0
            end
        ...
        # Constructing the region S12 by (2.2c) in Theorem 2.3
        indcol = min(n-2, n+1-ceil(Int, (n+1)/(1+1/r)))
        vOld = zeros(Ty,1,n)
        vNew = zeros(Ty,1,n)
        v = zeros(Ty,1,n)
        S = BandedMatrix(-1 => -r ./ (2*(1:n-1).-1), 0 => Fill(0, n), 1 => r ./ (2*(1:n-1) .+ 1))
        for j = indcol:-1:2
            i = d2[j]+1:n+1-j
            if length(i) > 0
                # Recurrence
                view(vOld, d2[j+2]-1:d2[j+2]) .= view(d, j+2,2:-1:1)
                view(v, d2[j+1]-1:d2[j+1]) .= view(d, j+1, 2:-1:1)
                view(vNew,i) .= view(S, i, d2[j]:n+2-j) * view(v, d2[j]:n+2-j)
                broadcast!(*, view(vNew,i), (2*j+1), view(vNew,i))
                broadcast!(+, view(vNew,i), view(vOld,i), view(vNew,i))
                # append to gamma
                axpy!(ge[j], view(vNew,i), view(gamma,i))
        ...
        # Constructing the first column
        i = 2:n-1
        view(vOld, d2[3]-1:d2[3]) .= view(d, 3,2:-1:1)
        view(v, d2[2]-1:d2[2]) .= view(d, 2, 2:-1:1)
        view(vNew,i) .= view(S, i, 1:n) * view(v, 1:n)
        broadcast!(*, vNew, 3, vNew)
        broadcast!(+, vNew, vOld, vNew)
        view(vNew,n) .= -r*3*view(v, n - 1)/(2*n - 3)
        axpy!(ge[1], view(vNew,2:n), view(gamma,2:n))
